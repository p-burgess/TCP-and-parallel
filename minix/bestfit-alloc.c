...
CHECKHOLES;
	...
	while (hp != NULL) {
		if (hp->h_len == clicks) {
			/* We found a hole that is the exact size, use it.*/
			old_base = temp_ptr->h_base;	/* remember start */
			temp_ptr->h_base += clicks;	/* bite a piece off */
			temp_ptr->h_len -= clicks;	/* ditto */
			break;
			}
		if (hp->h_len > clicks) {
			if (hp->len - clicks < left left == -1){
				left = hp->len - clicks;
				best = hp;
				prev_best = prev_ptr;
			}
	...
	if (left > -1){
		old_base = best->h_base;/* remember start */
...
CHECKHOLES;
	...
	if (temp_ptr->h_len == 0) del_slot(prev_ptr, temp_ptr);
