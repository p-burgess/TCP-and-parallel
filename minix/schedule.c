...
#define DEFAULT_USER_TIME_SLICE 300
#define SECONDARY_USER_TIME_SLICE 150

PRIVATE int DUTS = DEFAULT_USER_TIME_SLICE;
...
PRIVATE void balance_queues(struct timer *tp)
{
	...
	if (nr_procs < 15) {
		DUTS = DEFAULT_USER_TIME_SLICE;
	} else if (nr_procs < 75) {
		DUTS = SECONDARY_USER_TIME_SLICE;
	} else {
		DUTS = DEFAULT_USER_TIME_SLICE;
	}
	sched_set_timer(&sched_timer, balance_timeout, balance_queues, 0);
