 ...
 */
  register struct hole *hp, *prev_ptr, *best_ptr, *prev_best;
...
CHECKHOLES;
	...
	while (hp != NULL) {
		if (hp->h_len == clicks) {
            /* We found a hole that is exact match.  Use it. */
			remainder = 0;
			best_ptr = hp;
			prev_best = prev_ptr;
			break;
		...
		if (hp->h_len > clicks) {
		/* We found a hole big enough. */
            if (hp->h_len - clicks < remainder || remainder == -1){
                /* Keep hole with smallest remainder. */
				remainder = hp->h_len - clicks;
                best_ptr = hp;
                prev_best = prev_ptr;
            }
		}
	...
	if (remainder > -1){
		...
		old_base = best_ptr->h_base;     /* remember where it started */
		best_ptr->h_base += clicks;      /* bite a piece off */
		best_ptr->h_len -= clicks;     	 /* ditto */
		/* Return the start address of the acquired block. */
		return(old_base);
	}

	/* Delete the hole if used up completely. */
	if (best_ptr->h_len == 0) del_slot(prev_best, best_ptr);
