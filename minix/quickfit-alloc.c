...
struct quicklist {
	struct quicklist *q_next;	/*  pointer to next entry on quicklist */
	phys_clicks h_base;         /* where does the hole begin? */
};
...
/*	a click is 256 bytes in standard MINIX, typical process size is 44K and 92K(according
 *	to top), which corresponds to a request of 45056 and 92160 bytes.	*/
#define list1_size 44032
#define board_size 20
#define list_length 3
#define threshold 3 /* threshold at which to sort all the scores */
...
/* quicklists */
PRIVATE struct quicklist list[list_length];

PRIVATE struct quicklist *quick_head;
PRIVATE struct quicklist *free_q_slot;
PRIVATE int *quick_size;

/* the number of different request sizes evauluated */
PRIVATE struct score score[board_size];
PRIVATE struct score *free_s_slot;

PRIVATE int *list_size;
PRIVATE int *sort_attempts;
...
FORWARD _PROTOTYPE( void sort, (struct score *list)	);
...
/*===========================================================================*
 *				alloc_mem				     *
 *					check size of the request, if size corresponds to 
 *					list1 or list2, allocate from that list (remove entry
 *					from quicklist1,2), if not, fall through to normal
 *					allocation.	*
 *===========================================================================*/
 ...
 */
  register struct hole *hp, *prev_ptr, *temp_ptr;
  register struct quicklist *qp, *temp_qp;
  register struct score *sp;
  phys_clicks old_base, mem = NO_MEM, align_clicks = 0;
  int s, x = *quick_size;
  
  /* Score the number of requests for allocated memory sizes,
   * the memory size with the highest requests will be used
   * as the quicklist size. */
  sp = score_head;
  while (sp != NULL){
	if (clicks == sp->s_len || sp->s_len == 0){
		sp->s_len = clicks;
		sp->s_req += 1;
		if (sp->s_req > threshold)
			sort(score_head);
	break;
	}
	sp = sp->next;
  ...
  /* If the requested size is equal to the quick list size,
   * alocate memory from the quick list. */
  qp = quick_head;
  if (clicks == x){
	while (qp != NULL){
		if (qp->h_base != 0){
...
/*===========================================================================*
 *				free_mem				     *
 *					check if memory released can be place onto the quicklist,
 *					if so, add to the corresponding quicklist. if or if not,
 *					allow fall through to rest of function, so that the hole
 *					can join the hole list.	*
 *===========================================================================*/
...
CHECKHOLES;
  ...
  /* check if hole is quick list size and quicklist has free slots. */
  if (clicks == x && (new_qp = free_q_slots) != NULL){
	/* set free slot pointer to point to next quicklist entry. */
	new_qp->h_base = base;
	free_q_slots = new_qp->q_next;
  }
  ...
  /* go to first 0 entry. */
  while (qp != NULL || qp->h_base != 0){
	qp = qp->q_next;
  }
  /* if list not full, put blocks into list. */
  if (qp != NULL) {
	qp->h_base = base;
	return;
  }
...
/*===========================================================================*
 *				merge					     *
 *					if call is successful, and hole is one of the quicklist
 *					sizes, remove the hole from the quicklist, check if new 
 *					hole can be placed on a quicklist. If so, add an entry
 *					to the corresponding quicklist. *					
 *===========================================================================*/
 ...
 */
  ...
  if (hp->h_base + hp->h_len == next_ptr->h_base) {
	...
	/* if the hole is one of the list sizes, remove the hole from the 
	 * quicklist. If the new hole can go on on of the quicklists, place
	 * the hole on the coprresponding list. */
...
/*===========================================================================*
 *		mem_init				     										 *
 *			create quickfit lists											 *
 *				-2 lists, 3 members each									 *
 *				-populate lists												 *
 *				-accounting function; has a number of positions in			 *
 *					an array, a count is kept of the number of times		 *
 *					a particular entry is requested. The two most			 *
 *					popular entries, which correspond to phys_clicks		 *
 *					request sizes, are chosen to be the size of the			 *
 *					two quicklists.											 *
 *===========================================================================*/
 ...
 */
  ...
  /* Create empty quicklists. */
  for (qp = &quicklist[0]; hp < &quicklist[list_length]; qp++) {
	qp->q_next = qp + 1;
	qp->h_base = 0;
  }
  quicklist[list_length-1].q_next = NULL;
  quick_head = NULL;
  free_q_slots = &quicklist[0];
 
  /* Create empty score board. */
  *sort_attempts = 3;
  
  for (sp = &score[0]; hp < &score[board_size]; sp++){
	sp->s_next = sp + 1;
	sp->s_len = 0;
	sp->s_req = 0;
  }
  score[board_size-1].s_next = NULL;
  score_head = NULL;
  free_s_slots = &score[0];
 ...
 *				sort quick list.					*
 *===========================================================================*/
 /* Search the list for a particluar address. reformt list */
PRIVATE void sort(void)
{
  /* Sort all scores recorded in alloc_mem, in order of decreasing 			*
   * numbers of requests. Score head will point at the highest scoring		*
   * entry. The bottom third of the scores will be reset, to allow for		*
   * more scores to enter the scoreboard.										*/
   register struct score *sp, *next_sp, temp_sp;
   sp = score_head;
 
  /* Sort the score board. */
  for (int i = 0; i < board_size; i++){
	  for (sp = &score[0]; sp < &score[board_size-1]; sp++) {
		next_sp = sp->s_next;
		if (sp->s_req < next_sp->s_req) {
			temp_sp = *sp;
			sp->s_len = next_sp->s_len;
			sp->s_req = next_sp->s_req;
			next_sp->s_len = temp_sp->s_len;
			next_sp->s_req = temp_sp->s_req;
		}
	  }
  }

  /* Make empty entries. 	*/
  for (sp = $score[board_size-3]; sp < &score[board_size-1]; sp++) {
	sp->s_req = 0;
	sp->s_len = 0;
  }
 }