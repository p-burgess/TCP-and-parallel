...
/* the connect_queue holds file descriptors for the various threads to connect to. */
...
//	accept messages on the pipe of an integer file descriptor

//	copied from processor
	pthread_mutex_lock(&connq.control.mutex);
	while (connq.control.active) {
		// wait for something on the pipe
		while (connq.work.head==NULL && connq.control.active) {
			pthread_cond_wait(&connq.control.cond, &connq.control.mutex);
		}
		if (!connq.control.active) 
			break;
		//	we got something!
		//		read from the fifo
		//		create node
		//		place new node into (aq)ccept queue
		myprocess=(wnode *) queue_get(&connq.work);
		myprocess->processor=pthread_self();
		pthread_mutex_unlock(&aq.control.mutex);
		//process job
		pthread_mutex_lock(&rq.control.mutex);
...
void initialize_structs(void) {
	numthreads=0;
	numconnects=0;
	if (control_init(&connq.control))
		dabort();
	queue_init(&connq.work);
	if (control_init(&aq.control))
		control_destroy(&connq.control);
		dabort();
	queue_init(&aq.work);
	...
	control_activate(&connq.control);
