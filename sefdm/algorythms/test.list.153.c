...
void invert (table *mytable,table *augment)
{
	...
	double S,mag,real,imag,augreal,augimag,rreal,rimag,areal,aimag,pivotreal,pivotimag;
	S=0.0;
	...
	for (r=0;r<mytable->columns;r++) {
		...
		//if (pivotreal != 1.0) {
			for (i=r;i<mytable->columns;i++) {
				...
				Rr[i]->real = rreal/pivotreal;
				Rr[i]->imag = rimag/pivotreal;
				Ar[i]->real = areal/pivotreal;
				Ar[i]->imag = aimag/pivotreal;
				printf ("rreal: %.10e\t\trrimag: %.10e\n",Rr[i]->real,Rr[i]->imag);
