...
void node_set (node *mynode, double in_real, double in_imag)
{
	mynode->real=in_real;
	mynode->imag=in_imag;
}
...
void divide_array_by_double (node **myarray, int length,double denominator)
{
	int i;
	double real,imag;
	node *mynode;
	...
	{
		mynode = myarray[i];
		real = ceil(myarray[i]->real/denominator*1000000)/1000000;
		imag = ceil(myarray[i]->imag/denominator*1000000)/1000000;
		myarray[i]->real = real;
		myarray[i]->imag = imag;
	}
}
...
void copy_row_to_table (table *mytable,node **array,int row)
{
	unsigned int i;
	double real,imag;
	node *mynode,*prevnode,*next;//,*newnode;
	//printf ("\n%d\n",x);
	for (i=0;i<mytable->columns;i++) {
		prevnode = NULL;
		mynode = mytable->head[i];
		while (mynode != NULL) {
			if (mynode->row == row) {
				next = mynode->next;
				memcpy (mynode,array[i],sizeof(node));
				mynode->next = next;
				
				//if (prevnode ==  NULL) {
				//	mytable->head[i] = array[i];
				//} else {
				//	prevnode->next = array[i];
				//}//print_node (mynode);
				
				//array[i]->next = mynode->next;
				
				//mynode->real = array[i]->real;
				//mynode->imag = array[i]->imag;
...
void mult_array_by_node (node **row,node *node,int length)
{
	unsigned int i;
	double real,imag;
	//node *mynode;
...
void invert (table *mytable,table *augment)
{
	...
	node *pivotnode,*mynode,*anode,*conj,*xnode,**Rr,**Cr,**Ar,*snode,*asnode,**Arnorm;
	...
	// result of normalising Ar
	Arnorm = (node **) malloc (mytable->columns*sizeof(node *));
	for (i=0;i<mytable->columns;i++) {
		Ar[i] = (node *) malloc (sizeof(node));
	}
	...
	for (r=0;r<2;r++) { // 1 should be mytable->columns
		
		// a(r,x) = pivot row, a(r,r) = pivot element
		// Gaussian Elimination
		//	normalize pivot row (pivot entry = 1 + 0i)
		...
		//}
			node_init (conj,pivotreal,pivotimag,pivotnode->row,pivotnode->col);
			//print_node (conj);
		...
		//printf ("\nbefore multiplication by conjugate:\n");
		//printf("Rr[i]");print_array (Rr,mytable->columns);
		//printf("Ar[i]");print_array (Ar,augment->columns);
		
			mult_array_by_node (Rr,conj,mytable->columns);
			mult_array_by_node (Ar,conj,mytable->columns);

		//printf ("\nafter multiplication by %.10e + i%.10e:\n",conj->real,conj->imag);
		...
		divide_array_by_double (Rr,mytable->columns,pivotreal);
		divide_array_by_double (Ar,mytable->columns,pivotreal);
		
		//printf ("\nafter division by %.10e:\n",pivotreal);
...
printf ("\nhere");				
				...
				Arnorm[i]->real = rrreal;
				Arnorm[i]->imag = rrimag;
				
				Ar[i]->imag = rareal;
				Ar[i]->imag = raimag;
				node_set (Rr[i],rrreal,rrimag);
				node_set (Ar[i],rareal,raimag);
				
				printf ("Arnorm[%d]->real: %.10e\t\tArnorm[%d]->imag: %.10e\n",i,Arnorm[i]->real,i,Arnorm[i]->imag);
		...
		printf ("\nbefore row inserted into table:\n");
		print_table (A);
		//print_table (Aug);
		//printf("Rr[i]");print_array (Rr,mytable->columns);
		printf("Ar[i]");print_array (Ar,augment->columns);
