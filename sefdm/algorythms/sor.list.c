...
double *A_sor_x (int size, list *mylist, double *x, double *b, double o)
{
 int i,j,k=0;
 node *mynode;
 ...
 do {
  ...
  i = mynode->row;
  j = mynode->col;
  a[i][j]=mynode->G; // this could be held in log(size) places
 ...
 for (i=1;i<size;i++) {
  ...
  left_x[i] =0.0;
  left_x[i] +=(1-o)*x[i];
  left_x[i] +=o*(b[i]-right_x)/mylist->diag[i]; }
 return left_x;
}
