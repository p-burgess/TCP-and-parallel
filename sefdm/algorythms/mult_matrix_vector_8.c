...
void table_init (table *mytable,int in_rows,int in_columns,char *name)
{
	...
	for (i=0;i<in_columns;i++) {
		...
		for (j=0;j<in_rows;j++) {
			mynode = (node *) malloc (sizeof(node));
			real = sin(2*PI*ALPHA*(i-1)*j/mytable->rows)/sqrt(mytable->rows);
			imag = cos(2*PI*ALPHA*(i-1)*j/mytable->rows)/sqrt(mytable->rows);
			node_init (mynode,real,imag,j,i);
			// what happens when we move this function to here
...
void augment_matrix (table *mytable,table *myaugment)
{
	node *mynode,*augnode,*lastnode;
	unsigned int i,j;
	double x;
	for (i=0;i<mytable->columns;i++) {
		mynode = mytable->head[i];
		lastnode = (node *) malloc (sizeof(node));
		node_copy (lastnode,mytable->head[i]);
		myaugment->head[i] = lastnode;
		mynode = mynode->next;		
		while (mynode != NULL) {
			augnode = (node *) malloc (sizeof(node));
			node_copy (augnode,mynode);
			lastnode->next = augnode;
			lastnode = augnode;
			mynode = mynode->next;
		}
		myaugment->tail[i] = lastnode;
	}
	for (i=(mytable->columns);i<(myaugment->columns);i++) {
		lastnode = (node *) malloc (sizeof(node));
		if (i == (mytable->columns)) {x = 1.0;}
		else {x = 0.0;}
		node_init (lastnode,x,0,0,i);
		myaugment->head[i] = lastnode;
		for (j=1;j<mytable->rows;j++) {
			augnode = (node *) malloc (sizeof(node));
			if ((i-mytable->columns) == j) {x = 1.0;}
			else {x = 0.0;}
			node_init (augnode,x,0,j,i);
			lastnode->next = augnode;
			augnode->next = NULL;
			lastnode = augnode;
		}
...
int main()
{
	...
	table *SUBTRAHEND = (table *) malloc (sizeof(table));
	table_init (SUBTRAHEND,NUM_ROWS,(2*NUM_COLS),"SUBTRAHEND");
	print_table (SUBTRAHEND);
	
	//reduce_column calculate_subtrahend (AUGMENT,SUBTRAHEND,0);	
