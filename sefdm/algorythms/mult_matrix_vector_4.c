...
// and a random vector, then subtract
...
void create_pivot_matrix (table *mytable, unsigned int x,table *outtable)
{
	unsigned int i;
	double real;
	double imag;
	...
	for (i=0;i<mytable->rows;i++)
	{
		...
		while (cursor != NULL) {
			if (cursor->row != x) {
				...
				memcopy (cursor->real,&real,sizeof(double));
				memcopy (cursor->imag,&imag,sizeof(double));
			} else {
				;
			}
			cursor = cursor->next;
