...
void mult_row_by_conjugate_to_new_row (node **newrow,node **row,node *conj,int length)
{
	unsigned int i;
	double real,imag;
	node *mynode;
...
void invert (table *mytable,table *augment)
{
	...
	for (r=0;r<mytable->columns;r++) {
		...
		// 		multiply row and augment row by conjugate
			...
			mult_row_by_conjugate_to_new_row (Arnorm,Ar,conj,mytable->columns);
			printf("Arnorm[i]");
			print_array (Arnorm,mytable->columns);
			/*
			for (i=0;i<mytable->columns;i++) {
