...
void divide_array (node **myarray, int length,double denominator)
{
	int i;
	double real,imag;
	node *mynode;
	...
	{
		mynode = myarray[i];
		real = myarray[i]->real/denominator;
		imag = myarray[i]->imag/denominator;
		myarray[i]->real = real;
		myarray[i]->imag = imag;
	}
}
...
void mult_row_by_conjugate (node **row,node *conj,int length)
{
	unsigned int i;
	double real,imag;
	node *mynode;
	for (i=0;i<length;i++) {
		real = row[i]->real*conj->real - row[i]->imag*conj->imag;
		imag = row[i]->real*conj->imag + row[i]->imag*conj->real;
		row[i]->real = real;
		row[i]->imag = imag;
...
void invert (table *mytable,table *augment)
{
	...
	for (r=0;r<mytable->columns;r++) {
		...
		mult_row_by_conjugate (Ar,conj,mytable->columns);
		mult_row_by_conjugate (Ar,conj,mytable->columns);
		...
		divide_array (Rr,mytable->columns,pivotreal);
		divide_array (Ar,mytable->columns,pivotreal);
