...
void pivot_matrix (table *mysubtrahend,table *mytable, unsigned int x)
{
	unsigned int i,j,k;
...
*/	
	...
	//print_table (mytable);
	print_table (mysubtrahend);
	
	// for each row compute a new row which is the product of
	// its pivot element and the pivot row. 
	//node **sbnode = (node **) malloc (mytable->columns*sizeof(node *));
	
	// compute a new matrix the same size as input matrix
	// for every column inthat matrix, compute a
	// new column, each entry the product of the correspondongentry of the pivot row
	node *sbnode = (node *) malloc (mytable->columns*sizeof(node));
	...
	for (i=0;i<mytable->columns;i++) {
		...
		// mynode points to ith entry of pivot column
		mynode = (*(mytable->entry+i)+x);

		for (j=x;j<mytable->rows;j++) {
			//if (j!=x)
			...
			imag = (pivot_row[j]->real*mynode->real - pivot_row[j]->imag*mynode->imag);
				...
				node_init (&sbnode[i],real,imag,i,j);
				//print_node (sbnode[i]);
		}
	}
	//printf("\nhere\n");
	for (i=0;i<mytable->columns;i++) {
		//printf("%d,\t%d,\t : %.10e\t%.10e/t\n",sbnode[i]->row,sbnode[i]->col,sbnode[i]->real,sbnode[i]->imag);
	}
...
int main()
{
	...
	table_init (SUBTRAHEND,NUM_ROWS,NUM_COLS,"SUBYRAHEND");
	
	
	pivot_matrix (SUBTRAHEND,MATRIX,0);
