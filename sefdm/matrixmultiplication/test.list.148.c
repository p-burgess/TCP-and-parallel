...
void invert (table *mytable,table *augment)
{
	...
	double S,mag,real,imag,augreal,augimag,areal,aimag,pivotreal;
	S=0.0;
	...
	for (r=0;r<mytable->columns;r++) {
		...
		//		conjugate.
		if (pivotnode->imag != 0) {
			imag = pivotnode->imag*(-1.0);
			//printf ("\n\tpivot node has iaginary component\n")
		//}
			node_init (conj,pivotnode->real,imag,pivotnode->row,pivotnode->col);
			//print_node (conj);
		...
		if (pivotnode->real != 1.0) {
			for (i=r;i<mytable->columns;i++) {
				aimag = 0.0;
				areal = Rr[i]->real/pivotreal;				
				// ...divide augennted row by real part of pivot...
				if (Rr[i]->imag != 0.0) {
					aimag = Rr[i]->imag/pivotreal;
				} 
				Rr[i]->real = areal;
				Rr[i]->imag = aimag;
			}
			for (i=r;i<augment->columns;i++) {
				augimag = 0.0;
				augreal = Ar[i]->real/pivotreal;
				if (Ar[i]->imag != 0.0) {
					augimag = Ar[i]->imag/pivotreal;
				}
				Ar[i]->real = augreal;
				Ar[i]->imag = augimag;
			}
