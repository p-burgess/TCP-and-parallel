...
void pivot_matrix (table *mytable, unsigned int x,table *outtable)
{
	unsigned int i;
	double real;
	double imag;
	node *pivot,*pivot_column;
	node **pivot_row;
	node *cursor,*target;
	
	pivot_row = (node**) malloc (mytable->rows*(sizeof(node *)));
	for (i=0;i<mytable->columns;i++) {
		pivot_row[i] = (node *) malloc (sizeof(node));
		pivot_row[i] = mytable->head[i];
		while (pivot_row[i]->row != x) {
			pivot_row[i] = pivot_row[i]->next;
		} 
	}
...
int main()
{
	...
	table *SUBT = (table *) malloc (sizeof(table));
	table_init (SUBT,NUM_ROWS,NUM_COLS,"SUBTRAHEND");
	create_identity
	
	pivot_matrix (table *mytable,unsigned int pivot,);
